...
const getFileToLoad = (argv: yargs.Argv["argv"]): FileToLoad => {
  let mainFilename = defaultFilename;
  if (argv.f || argv.file) {
    if (argv.f && argv.file) {
      throw exitWithError(ErrCode.CliError, `-f and --file cannot be used at the same time`);
    }
  ...
  throw exitWithError(ErrCode.CliError, `command file not found, tried ${filesToTry.join(", ")}`);
};
...
export const mainAsync = async (argv: yargs.Argv["argv"]) => {
  resetColors();
